...
namespace DockerizedTesting.ImageProviders
{
    ...
    /// </summary>
    public class DockerProjectImageProvider : IDockerImageProvider
    {
        ...
        /// Use like this: DockerProjectImageProvider(nameof(YourProject))</param>
        /// <param name="projectLocator"></param>
        public DockerProjectImageProvider(string name, IProjectLocator projectLocator = null) :
            this((projectLocator ?? new ProjectLocator()).GetDockerProject(name))
        { }

        /// <summary>
        /// Builds a dockerized .NET project and provides the docker image which can then be used in integration tests. If possible don't use this approach as it can be brittle
        /// </summary>
        /// <param name="projectFile">The project file to build</param>
        public DockerProjectImageProvider(FileInfo projectFile)
        {
        ...
        private string buildDockerProject(FileInfo fileInfo)
        {
            ...
            var tag = Regex.Replace(fileInfo.Name.ToLower()
                          ...
                          , "[^a-z0-9]", string.Empty) + "_" + "dockerized_testing_" + this.getEpoch();
            ...
            {
                throw new DockerBuildFailedException(process.StandardError.ReadToEnd());
            }
