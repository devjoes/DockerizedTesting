    ...
    public class ProjectLocator : IProjectLocator
    {
        ...
        public IEnumerable<(string path, string[] names)> GetProjectNamesFromSln(string slnPath,
            ...
            this.getProjectPathsFromSln(slnPath)
                .Select(path => new {path, projectProps = extractProjectProperties(path)})
        ...
        public IEnumerable<FileInfo> FindSlnFiles()
        {
            var curDir = GetCurrentDir();
            return this.searchForSlnFiles(curDir);
        }
        ...
        {
            foreach (var sln in this.FindSlnFiles())
            {
        ...
        private IEnumerable<FileInfo> searchForSlnFiles(DirectoryInfo dir)
        {
            foreach (var file in dir.GetFiles("*.sln").Concat(dir.Parent == null ? Array.Empty<FileInfo>() : this.searchForSlnFiles(dir.Parent)))
            {
                yield return file;
            }
        }
